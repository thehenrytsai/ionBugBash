...
async function generateIonValues (): Promise<void> {
    ...
    const recoveryKeys = await IonKey.generateEs256kOperationKeyPair();
    const updateKeys = await IonKey.generateEs256kOperationKeyPair();
    ...
    const did = IonDid.createLongFormDid({recoveryKey: recoveryKeys[0], updateKey: updateKeys[0], document: document});
    ...
    const request = IonRequest.createCreateRequest({recoveryKey: recoveryKeys[0], updateKey: updateKeys[0], document: document});
